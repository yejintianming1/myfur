...
import com.wu.fur.es.annotations.EsReader;
...
//        client = new RestHighLevelClient(RestClient.builder(new HttpHost("localhost",9200)));
    ...
    public boolean createIndex(Class clazz) throws IOException {
        ...
        request.settings(Settings.builder()
                ...
                .put("index.number_of_replicas", esSourceConfig.getReplicas()));
        //创建索引时创建文档类型映射
...
//        request.mapping(builder);
        request.mapping(mappingJson,XContentType.JSON);//JSON.toJSONString(t, SerializerFeature.WriteNullStringAsEmpty)
    ...
    public <T,Q,E,O,R,W> EsPagination<T> search(Q terms,E matches,O orTerms,R orMatches,W prefixItems,SortField[] sortFields,EsPage page,Class<T> clazz) throws IOException {
        ...
        if (null != hits) {
            ...
            for (SearchHit searchHit : searchResponse.getHits()) {
                ...
                String sourceSring = JSONObject.toJSONString(sourceMap);
                responseList.add(JSONObject.parseObject(sourceSring, clazz));
            }
